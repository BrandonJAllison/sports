...
const CFL = () => {
    ...
    useEffect(() => {
        const getInfo = () => {
            axios
                .get('http://demo0337722.mockable.io/CFL')
                .then(response => {
    ...
    if (gameInfo.length !==0) {
        if (viewAll) {
            ...
            return (gameInfo.cfl.map(el => <UGameCard gameInfo={el} />))
        } else {
           
            return (gameInfo.cfl.slice(0, 3).map(el => <UGameCard gameInfo={el} />));
        }
    } else{
            ...
            <div style={{ display: "flex" }}>
                <div className="UGameContainer" style={{ background: "grey", minWidth: "90vw", minHeight: "20vw"}}>
                    ...
                    <div style={{display:"flex", justifyContent:"center"}}>
                        <BotButton onClick={()=> {setViewAll(!viewAll)}}>View {expand()} CFL Games</BotButton>
                    </div>
                </div>
            </div>
